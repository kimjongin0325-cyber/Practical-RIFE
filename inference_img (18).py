# =====================================================
# ✅ [코랩 최적화 RIFE Interpolation Loop v6.4-FINAL]
# - RIFE_HDv3.py 사용 + v3.1 호환 + scale/timestep 지원
...
opt = {
    ...
    "demo_dir": os.path.join(BASE_DIR, "demo"),  # 사용 안 함
...
print("⚡ RIFE 모델 로딩 중...")
rife_model = Model()
rife_model.load_model(opt["model_path"])  # ✅ RIFE_HDv3의 load_model 사용
rife_model.eval()
rife_model.device()
print("✅ 모델 로딩 완료.")
...
def run_rife_inference(img0_bgr, img1_bgr, timestep=0.5):
    ...
    with torch.no_grad():
        # ✅ RIFE_HDv3의 inference 호출, scale=1/timestep으로 조정
        out_img = rife_model.inference(I0, I1, scale=1.0/timestep)

    out_img_np = (out_img[0].cpu().numpy().transpose(1, 2, 0) * 255.0).clip(0, 255).astype(np.uint8)
...
os.makedirs(opt["demo_dir"], exist_ok=True)
