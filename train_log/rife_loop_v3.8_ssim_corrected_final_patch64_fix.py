# =====================================================
# ✅ [RIFE Interpolation Loop v6.6-SSIM-Corrected-Final Patch64-Fix]
# - 실전용 / Colab, Kaggle 완전 호환
# - 64배수 패딩 및 downscaled 오류 수정
...
sys.path.extend([
    ...
    os.path.join(BASE_DIR, "model", "pytorch_msssim")
])
...
def run_rife_inference(img0_bgr, img1_bgr, timestep=0.5):
    ...
    h, w, _ = img0_bgr.shape
    ph = ((h // 64) + 1) * 64 if h % 64 != 0 else h
    pw = ((w // 64) + 1) * 64 if w % 64 != 0 else w
    if (ph != h) or (pw != w):
        img0_bgr = cv2.copyMakeBorder(img0_bgr, 0, ph - h, 0, pw - w, cv2.BORDER_REFLECT_101)
        img1_bgr = cv2.copyMakeBorder(img1_bgr, 0, ph - h, 0, pw - w, cv2.BORDER_REFLECT_101)

    original_h, original_w = h, w
    if h > 1080:
        downscaled = True
        img0_bgr = cv2.resize(img0_bgr, (1920, 1080), interpolation=cv2.INTER_AREA)
        img1_bgr = cv2.resize(img1_bgr, (1920, 1080), interpolation=cv2.INTER_AREA)
