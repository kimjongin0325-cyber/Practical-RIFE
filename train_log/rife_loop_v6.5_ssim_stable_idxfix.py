# =====================================================
# ✅ [실전용 RIFE Interpolation Loop v6.5-SSIM-Stable-IdxFix]
# - Colab/Kaggle 완전 호환
# - 1000장 이상 대량 프레임 안정 지원
# - SSIM 품질 평가 + 인덱스 불연속 수정
# =====================================================
...
try:
    ...
    raise ImportError(f"🚨 Model import 실패: {e}")
...
def run_rife_inference(img0_bgr, img1_bgr, timestep=0.5):
    ...
    if ssim_loss is not None:
        try:
            ...
            ssim_value = ssim_loss(out_tensor, img1_tensor).item()
        except Exception as e:
...
# -------------------- [7] 메인 루프 (인덱스 수정본) --------------------
success, idx = 0, 0
shutil.copy(frame_files[0], os.path.join(OUTPUT_DIR, f"img{idx:05d}.png"))

for i in range(num_frames - 1):
    if i not in preloaded:
        print(f"⚠️ {i}번째 쌍 누락 — 보간 건너뛰고 원본 복사")
        idx += opt["scale"]
    ...
    for j in range(1, opt["scale"]):
        ...
        if ssim_value is not None:
            print(f"✅ 프레임 저장: {os.path.basename(dst)} (SSIM: {ssim_value:.4f})")
        else:
            print(f"✅ 프레임 저장: {os.path.basename(dst)} (SSIM 계산 안 됨)")
        success += 1
        torch.cuda.empty_cache()
